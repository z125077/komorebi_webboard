...
$posts_data = [
    ...
    $stmt = $pdo->prepare("INSERT IGNORE INTO posts (post_id, title, content, author_id, category, created_at, views, likes) VALUES (?, ?, ?, ?, ?, ?, ?, ?)");
    foreach ($posts_data as $post) {
        $stmt->execute($post);
    }

    echo "Sample posts inserted successfully!<br>";
    echo "Database initialization complete! You can now access the webboard.";
